...
def _mesh_capture(context, path=True):
    ...
    try:
        ...
        if path:
            from . import finger_range, finger_layout
            ...
            for probe, sample in ((base, rest), (bm, current)):
                try:
                    ...
                    discovered = finger_range.discover(probe, faces, finger_layout._quad_band, obj.matrix_world)
...
def _validate_mesh(obj, record, basis, mesh=None):
...
def _reference_mesh(context, obj, record):
    """An owned layout may still use its original, verified definition snapshot."""
    from . import finger_layout
    layout = finger_layout.state(context)
    if layout.mesh == obj and layout.source and layout.applied and layout.record:
        recipe = json.loads(layout.record)
        if recipe.get('definition_id'):
            original = _snapshot(obj, record['basis_key'], layout.source)
            try: matches = _topology(original) == record['topology']
            finally: original.free()
            if not matches: return None
            if finger_layout.fingerprint(obj) != layout.signature:
                raise ValueError('The generated mesh changed. Recapture the definition before new actions.')
...
def frame(context, *, require_basis=False, require_bend=False, require_confirmed=False):
    ...
    if require_basis and not s.use_basis:
        raise ValueError('This is a current-key preview. Use Basis Reference and confirm before changing topology or bone axes.')
    if record['kind'] == 'MESH':
        _validate_mesh(s.source, record, s.use_basis, _reference_mesh(context, s.source, record))
    else:
    ...
    if s.bend_record:
        try:
            ...
            _validate_mesh(s.bend_source, top, s.use_basis, _reference_mesh(context, s.bend_source, top))
...
def basis_reference(context):
    ...
    record = json.loads(s.record)
    if record['kind'] == 'MESH': _validate_mesh(s.source, record, True, _reference_mesh(context, s.source, record))
...
def clear(context):
    ...
    s.record = s.bend_record = s.pending = s.revision = ''
    s.confirmed = False
