...
def cached_frame(context):
    ...
    key = (context.scene.as_pointer(), s.source.as_pointer() if s.source else 0, s.revision, s.use_basis,
...
def _refresh():
    ...
    scene, key = request
    try: value, error = definition.frame(SimpleNamespace(scene=scene)), ''
...
def _drawable():
    ...
    return cached_frame(bpy.context)[0]
...
def _draw_labels():
    ...
    labels = [(data['root'], 'Start', (1., .68, .12, 1.)),
                                (data['tip'], 'End', (.15, 1., .75, 1.)),
                                (data['midpoint'], 'Basis' if data['basis'] else f"{data['key']} (preview)", (.9, .9, .9, 1.))] if data else []
...
class CHARACTERDESIGNER_OT_finger_definition(Operator):
    ...
    def execute(self, context):
        s = definition.state(context)
        try:
            if self.action == 'CAPTURE': definition.capture(context)
            ...
            elif self.action == 'TOP': definition.set_top(context)
...
def draw_controls(layout, context):
    s = definition.state(context)
    box = layout.box()
    box.label(text='Finger Definition', icon='ORIENTATION_NORMAL')
    row = box.row(align=True)
    capture = row.row(align=True)
    capture.enabled = context.mode in {'EDIT_MESH', 'EDIT_ARMATURE', 'POSE'}
    capture.operator('character_designer.finger_definition', text='Capture Selection', icon='EYEDROPPER').action = 'CAPTURE'
    if s.record or s.pending: row.operator('character_designer.finger_definition', text='', icon='X').action = 'CLEAR'
    if context.mode == 'EDIT_MESH':
        row = box.row(align=True)
        row.operator('character_designer.finger_definition', text='Mark Start').action = 'START'
        row.operator('character_designer.finger_definition', text='Mark End').action = 'END'
    if s.record:
        ...
        if data:
            ...
            box.label(text=f"Reference: {'Basis' if data['basis'] else data['key']+' (preview)'}")
            box.label(text=f'Length: {length}')
            if data['direction_label'] == 'Review arrow': box.label(text='Direction needs confirmation.', icon='INFO')
            box.label(text='Bend: top surface' if data['bend'] is not None else 'Bend: undefined (optional)')
            if not data['basis']:
                box.operator('character_designer.finger_definition', text='Use Basis Reference').action = 'BASIS'
            row = box.row(align=True)
            row.operator('character_designer.finger_definition', text='Confirmed' if s.confirmed else 'Confirm', icon='CHECKMARK').action = 'CONFIRM'
            row.operator('character_designer.finger_definition', text='Swap Ends').action = 'SWAP'
            if context.mode == 'EDIT_MESH':
                box.operator('character_designer.finger_definition', text='Set Top Surface (optional)').action = 'TOP'
            if data['bend'] is not None: box.prop(s, 'flip_bend')
        elif error:
            import textwrap
            for line in textwrap.wrap(error, width=43): box.label(text=line)
        row = box.row(align=True)
        row.operator('character_designer.finger_definition', text='Show', icon='HIDE_OFF').action = 'SHOW'
        row.operator('character_designer.finger_definition', text='Hide', icon='HIDE_ON').action = 'HIDE'
    else:
        box.label(text='Select a surface path, edges or a bone chain.')
        box.label(text='Definition only; no geometry changes.')
    if s.status:
        import textwrap
