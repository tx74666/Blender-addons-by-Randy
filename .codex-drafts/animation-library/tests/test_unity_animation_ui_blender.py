...
class Layout:
    ...
    def template_list(self, listtype, listid, data, prop, active_data, active_prop, **kwargs):
...
def test_operators_draw_restore_and_cancel(folder):
    ...
    first = draw()
    assert "character_designer.animation_library" in first.buttons
    assert "character_designer.animation_unity_import" not in first.buttons
