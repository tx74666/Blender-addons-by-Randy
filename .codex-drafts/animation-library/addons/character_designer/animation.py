...
from . import animation_library, animation_library_ui
...
def _target(context):
    ...
    if obj and obj.type == "ARMATURE" and not obj.get("character_designer_motion_preview"):
        return obj
    return animation_library.target(context)
...
def register_animation_runtime():
    bpy.types.Scene.character_designer_animation_library = PointerProperty(type=animation_library.CDAnimationLibrary)
...
def unregister_animation_runtime():
    stop_animation_runtime()
    if hasattr(bpy.types.Scene, 'character_designer_animation_library'):
        del bpy.types.Scene.character_designer_animation_library
...
class CHARACTERDESIGNER_PT_animation(Panel):
    ...
    def draw(self, context):
        ...
        layout.enabled = not preview and not animation_library.state(context).session
...
ANIMATION_CLASSES = (
    *animation_library.CLASSES,
    *animation_library_ui.CLASSES,
